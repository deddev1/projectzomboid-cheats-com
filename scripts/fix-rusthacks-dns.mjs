/**
 * Attach projectzomboidcheats.com to Worker islecheat and create DNS CNAMEs.
...
const DOMAINS = ['projectzomboidcheats.com', 'www.projectzomboidcheats.com'];
...
async function ensureCname(name, content) {
	const fqdn = name.includes('.') ? name : `${name}.projectzomboidcheats.com`;
	const existing = await listDns();
	const hit = existing.find(
		(r) => r.name === fqdn || r.name === name || (name === '@' && r.name === 'projectzomboidcheats.com'),
	);
	if (hit) {
	...
	const recordName = name === 'projectzomboidcheats.com' || name === '@' ? 'projectzomboidcheats.com' : name;
...
async function main() {
	...
	await ensureCname('projectzomboidcheats.com', TARGET);
